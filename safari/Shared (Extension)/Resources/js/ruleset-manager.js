...
async function pruneInvalidRegexRules(realm, rulesIn) {
    ...
    // Helper function to modify quantifiers
    function modifyQuantifiers(regex) {
        return regex.replace(/(\{[0-9,]*)(})/g, '$1 $2');
    }
    ...
    for (const rule of rulesIn) {
        ...
        toCheck.push(
            dnr.isRegexSupported({ regex, isCaseSensitive }).then(async (result) => {
                if (result.isSupported) {
                    return true;
                }
                
                // If the regex is unsupported, check for quantifiers and modify
                const modifiedRegex = modifyQuantifiers(regex);
                if (modifiedRegex !== regex) {
                    const modifiedResult = await dnr.isRegexSupported({ regex: modifiedRegex, isCaseSensitive });
                    if (modifiedResult.isSupported) {
                        rule.condition.regexFilter = modifiedRegex; // Update the rule with the modified regex
                        return true;
                    }
                }
